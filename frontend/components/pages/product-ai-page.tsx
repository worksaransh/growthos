...
const QUICK_PROMPTS = [
  ...
  "Which products have the worst return rates?",
  "Recommend bundles based on purchase patterns",
...
export function ProductAiPage() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    setLoading(true);
    try {
      const { response } = await api.specialistChat("product", msg);
      const aiMsg: Message = {
        id: crypto.randomUUID(),
        role: "assistant",
        content: response,
    ...
    <div className="p-6 flex flex-col gap-4 h-full overflow-hidden">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          >
            <AppIcon name="inventory_2" size={24} style={{ color: ACCENT }} />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[#F0F4FF]">Product AI</h1>
      ...
      <div className="glass-card p-4 flex-shrink-0">
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 bg-[#0F1520] border border-[#1E2737] rounded-lg px-3 py-2">
            <span className="text-xs text-[#8A95B0]">Total SKUs</span>
            <span className="text-sm font-bold text-[#F0F4FF]">127</span>
          </div>
          <div className="flex items-center gap-2 bg-[#0F1520] border border-[#1E2737] rounded-lg px-3 py-2">
            <span className="text-xs text-[#8A95B0]">Low Stock SKUs</span>
            <span className="text-sm font-bold" style={{ color: "#ef4444" }}>
              14
            </span>
          </div>
          <div className="flex items-center gap-2 bg-[#0F1520] border border-[#1E2737] rounded-lg px-3 py-2">
            <span className="text-xs text-[#8A95B0]">Avg Margin</span>
            <span className="text-sm font-bold" style={{ color: "#4ade80" }}>
              42%
            </span>
          </div>
          <div className="flex items-center gap-2 bg-[#0F1520] border border-[#1E2737] rounded-lg px-3 py-2">
            <span className="text-xs text-[#8A95B0]">Return Rate</span>
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {messages.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
              ...
              >
                <AppIcon name="inventory_2" size={38} style={{ color: ACCENT }} />
              </div>
              <p className="text-[#8A95B0] text-sm max-w-xs">
                Ask me anything about your product catalog, inventory, margins,
                or return rates.
              </p>
            </div>
          )}
            ...
            >
              {msg.role === "assistant" && (
                <div
                  ...
                  style={{ background: `${ACCENT}15` }}
                >
                  <AppIcon name="inventory_2" size={14} style={{ color: ACCENT }} />
                </div>
              )}
              <div
